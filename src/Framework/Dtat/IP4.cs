...
public static class IP4 : object
{
	...
	{
		var ipAddress = IPAddress.Parse(ipString: ipString);

		if (ipAddress is null)
		{
			var errorMessage = "IP4 Address is not valid!";
	...
	{
		var bytes = BitConverter.GetBytes(value: ipLong);
		...
		var result = new IPAddress(address: bytes).ToString();
